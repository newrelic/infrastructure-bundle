...
echo "Scanning Docker image ${IMAGE}:${TAG} ..."
docker run -t -e "SNYK_TOKEN=${SNYK_TOKEN}" -v ${PWD}/workspace:/project -v "/var/run/docker.sock:/var/run/docker.sock" snyk/snyk-cli:docker monitor --docker "${IMAGE}:${TAG}" --severity-threshold=high --org=ohai  --project-name="newrelic/infrastructure-bundle"
