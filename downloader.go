...
func (conf *config) expand(useStaging, overrideLatest bool) error {
	...
	var err error
	// Build template for staging URL
	conf.urlTemplate, err = template.New("url").Parse(conf.URL)
	if err != nil {
		return fmt.Errorf("error evaluating global URL template: %v", err)
	}

	// Build template repo
	conf.repoTemplate, err = template.New("repo").Parse(conf.Repo)
	if err != nil {
		return fmt.Errorf("error evaluating global URL template: %v", err)
	}
...
func (i *integration) expand(defaults *integrationConfig) error {
	...
	if i.URL != "" {
		...
		if err != nil {
			return fmt.Errorf("error building custom url template: %v", err)
		}
	} else {
	...
	if i.Repo != "" {
		...
		if err != nil {
			return fmt.Errorf("error building custom repo template: %v", err)
		}
	} else {
